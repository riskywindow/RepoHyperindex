...
tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/hyperbench-semantic-smoke.XXXXXX")"
bundle_dir="$tmp_root/bundle"
fixture_dir="$tmp_root/fixture-smoke"
daemon_dir="$tmp_root/daemon-semantic-smoke"
...
printf 'Running fixture semantic smoke baseline...\n'
...
printf 'Running daemon-backed semantic smoke candidate...\n'
UV_CACHE_DIR=/tmp/uv-cache uv run hyperbench run \
