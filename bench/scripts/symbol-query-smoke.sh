...
tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/hyperbench-symbol-smoke.XXXXXX")"
bundle_dir="$tmp_root/bundle"
fixture_dir="$tmp_root/fixture-smoke"
daemon_dir="$tmp_root/daemon-smoke"
...
printf 'Running fixture symbol smoke baseline...\n'
...
printf 'Running daemon-backed symbol smoke candidate...\n'
UV_CACHE_DIR=/tmp/uv-cache uv run hyperbench run \
  --adapter daemon \
